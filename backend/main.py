...
MODEL_PATH = Path(__file__).resolve().parent.parent / "ml_model" / "saved_models" / "best_knn_model.pkl"
...
def predict_water_activity(data: PredictionInput):
    distance_window = [data.distance] * 30
    slope_window = [0.0] * 30
    features = np.array([distance_window + slope_window])
    
    prediction = ml_model.predict(features)[0]

    confidence = 1.0
    if hasattr(ml_model, "predict_proba"):
        confidence = float(np.max(ml_model.predict_proba(features)))
